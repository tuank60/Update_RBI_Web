...
class CA_Flammable: #LEVEL 1
    def __init__(self,FLUID,FLUID_PHASE,MITIGATION_SYSTEM,proposalID,STORED_TEMP,API_COMPONENT_TYPE_NAME,toxic_percent):
        self.FLUID = FLUID
    ...
    def ca_cmdn_inst(self, select , i): #done
        try:
            x = self.mass_n(i) * self.toxic_percent/100
            if(self.eneff_n(i) == 0):
                return 0
            else:
